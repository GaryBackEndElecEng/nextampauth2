...
type mainPopCountryType = {
  ...
  loaded: boolean,
  setWorldLoaded: React.Dispatch<React.SetStateAction<boolean>>,
  worldLoaded: boolean,
  setNALoaded: React.Dispatch<React.SetStateAction<boolean>>,
  nALoaded: boolean,
  setSALoaded: React.Dispatch<React.SetStateAction<boolean>>,
  sALoaded: boolean,
  setAfrLoaded: React.Dispatch<React.SetStateAction<boolean>>,
  afrLoaded: boolean,
  setEurLoaded: React.Dispatch<React.SetStateAction<boolean>>,
  eurLoaded: boolean,
  setAsiaLoaded: React.Dispatch<React.SetStateAction<boolean>>,
  asiaLoaded: boolean,
}
...
export const PopulationContextProvider = (props: any) => {
  ...
  const [worldLoaded, setWorldLoaded] = React.useState<boolean>(false);
  const [nALoaded, setNALoaded] = React.useState<boolean>(false);
  const [sALoaded, setSALoaded] = React.useState<boolean>(false);
  const [afrLoaded, setAfrLoaded] = React.useState<boolean>(false);
  const [eurLoaded, setEurLoaded] = React.useState<boolean>(false);
  const [asiaLoaded, setAsiaLoaded] = React.useState<boolean>(false);

  React.useEffect(() => {
    if (asiaLoaded && eurLoaded && afrLoaded && sALoaded && nALoaded && worldLoaded) {
      setLoaded(true);
    }
  }, [asiaLoaded, eurLoaded, afrLoaded, sALoaded, nALoaded, worldLoaded]);

  return (
    <PopulationContext.Provider value={{ loaded, setLoaded, worldLoaded, setWorldLoaded, nALoaded, setNALoaded, sALoaded, setSALoaded, afrLoaded, setAfrLoaded, eurLoaded, setEurLoaded, asiaLoaded, setAsiaLoaded }}>
      {props.children}
