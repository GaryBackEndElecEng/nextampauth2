...
import type { userTypeNoAcc, DataType, userAccountType, PostDataType, testPosts, userInfoType, mainPageHit, mainGetSignup } from "@component/context/type";
...
export async function getSignups(): Promise<mainGetSignup[] | undefined> {
  const signups: mainGetSignup[] = await prisma.signup.findMany();
  await prisma.$disconnect();
  if (signups) {
    return signups
  }
}
export async function getPageHits(): Promise<mainPageHit[] | undefined> {
  const pageHits: mainPageHit[] = await prisma.pageHit.findMany();
  await prisma.$disconnect();
  if (pageHits) {
    return pageHits
  }
