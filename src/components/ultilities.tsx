    ...
    return (
        <div className="relative flex flex-col items-center my-2 mx-1 group relative ">
            ...
            <button className="m-auto p-1  rounded-lg  text-sm bg-black-300 text-black bg-white shadow-md shadow-green inline-flex " onClick={(e) => handleSignout(e)}>
                ...
                {session?.user?.image && <Image src={session.user.image} width={20} height={20} alt="www.masterconnect.ca" className="rounded-[50%] bg-none" />}
            </button>
            {session?.user?.name && <div className="text-xs bg-white p-1 rounded-lg my-1">{session.user.name.split(" ")[0]}</div>}
